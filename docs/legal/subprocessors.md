...
|---|---|---|---|---|---|
| **Firebase Cloud Messaging** (with the Apple Push Notification service for iOS) | Google Ireland Ltd. / Google LLC; Apple Distribution International Ltd. | Delivery of push notifications to the PlusOne app for iOS and Android | Push token bound to a login session, optional device label, and the notification message: internal identifiers and the kind of event only (e.g. a request id and "guest request created"), never guest names or contact details; the visible text is generic ("New guest request") [whether the event name may appear — decision pending] | Google and Apple global infrastructure | EU–US Data Privacy Framework; SCCs |
...
| **Resend** (extension — under consideration, not scheduled) | see section B | Transactional e-mail to guests on the venue's behalf, e.g. confirmation of the venue's decision on a request. The current product sends guests no messages; this row exists so venues are not surprised if that changes | Requester's e-mail address and the decision message | EU — Ireland | as section B |
