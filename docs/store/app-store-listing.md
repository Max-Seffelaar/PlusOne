...
> Gasten sturen een aanvraag, hosts vragen extra quotum — beide landen in de Aanvragen-tab, met een pushmelding zodra er iets wacht.
>
...
> gastenlijst,deur,check-in,venue,evenement,quotum,nachtclub,vip,portier,plus one
...
**Keywords** (≤100 chars, currently 90)

> guest list,door,check-in,venue,event,quota,nightclub,vip,bouncer,rsvp,plus one,event staff
...
Mirror `/privacy` from L1 once live. Data types to declare: contact info (email — account), user content (guest names/tiers entered by staff, processed as a data processor on behalf of the venue — see `docs/legal/data-processing-agreement.md`), identifiers (device push token). None used for tracking (no cross-app/cross-site tracking, no ad networks, no data broker sale — consistent with decision #36, no outbound marketing). Data linked to identity: account email, guest records (linked to the venue, not sold or shared across venues).
...
- Guideline 4.2 (webview wrapper) defense: native push (FCM/APNs) + an offline-capable door check-in flow that queues and syncs, both genuinely native-dependent behavior beyond a bare web wrapper.
