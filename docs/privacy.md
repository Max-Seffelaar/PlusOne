...
| 2 | **Landingpage-aanvragen** | Aanvragers | Naam; *optioneel* e-mail, telefoon, motivatie; beslissing + reden; sha256-hash van de status-URL-token (#43) | Toestemming/aanvraag betrokkene | Idem (event-geankerd), dan anonimisering (incl. nullen van de token-hash = auto-revocatie van de status-URL) |
| 2b | **Aanvraaglink-statistiek** (#43) | — (geen betrokkenen) | **Geen persoonsgegevens**: cookie-loze dag-aggregaten per link (`request_link_pageviews_daily`: link, dag, teller). Geen IP, UA of bezoekers-ID; "weergaven" = page loads, geen unieke bezoekers | n.v.t. (geen PII) | Onbeperkt (statistiek) |
