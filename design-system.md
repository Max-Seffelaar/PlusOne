...
**Waarom dichtheid op de pointer:** iPad landscape heeft desktopbreedte en een vinger. `lg:h-[30px]` gaf daar 30px-knoppen en typbare datumvelden. Kleiner dan 44px mag alleen achter `(pointer: fine)` (muis/trackpad), en dat matcht een iPad nooit. De datum/tijd-velden kiezen hun desktopmodus (typen, half-uur-dropdown) op `(min-width: 1024px) and (pointer: fine)`; op touch opent de kalender gecentreerd en blijft het native tijdwiel. Hetzelfde geldt voor de kit-InfoTip: popover alleen met een fijne pointer, anders de onderrand-sheet (max 560px). Bewaakt door `tests/unit/touch-density.test.ts` (zonder uitzonderingen sinds T1 sessie 2); de content-regel (`md:`, nooit een kale `lg:` in een scherm) door `tests/unit/tablet-content-breakpoint.test.ts`.

**Visueel kleiner dan 44px mag, het tikvlak niet.** Een chip of tekstknop die er klein uit moet zien krijgt een onzichtbare hit-ring (`before:absolute before:-inset-y-[…]`, de techniek van kit-`hitArea44`), die binnen de eigen rij-padding blijft en de buurman niet overlapt (rijen die wrappen staan daarom ver genoeg uit elkaar). Zit het element in een `overflow`-scroller, dan krijgt de scroller padding ter grootte van de ring, anders wordt hij afgeknipt. De ringen staan als geëxporteerde constanten in `kit.tsx` (`hitArea44`, `hitRing2`, `hitRingY2`/`Y4`/`Y5`/`Y6`/`Y13`); een scherm importeert er één en schrijft de class-string nooit zelf uit. Kies de ring op de gemeten hoogte in de browser (rand meegeteld), niet op een schatting: `kit.tap-target.test.tsx` pint per ring de maat waarvoor hij gekozen is.
