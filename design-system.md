...
**Visueel kleiner dan 44px mag, het tikvlak niet.** Een chip of tekstknop die er klein uit moet zien krijgt een onzichtbare hit-ring (`before:absolute before:-inset-y-[…]`, de techniek van kit-`hitArea44`), die binnen de eigen rij-padding blijft en de buurman niet overlapt (rijen die wrappen staan daarom ver genoeg uit elkaar). Zit het element in een `overflow`-scroller, dan krijgt de scroller padding ter grootte van de ring, anders wordt hij afgeknipt.
