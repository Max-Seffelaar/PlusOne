...
- **One responsive surface:** the `po` app at `/app` (`ResponsiveShell` — mobile bottom-tabs <1024px, desktop sidebar ≥1024px; ONE chrome breakpoint, no tablet tier — see `design-system.md` "Breakpoints & tablet"). The Deur tab picks its variant by plan decision 14, not by chrome: `(pointer: coarse)` OR <1024px gets the offline-outbox door (#25), only a fine pointer at ≥1024px gets the online-only cockpit. Every authenticated entry lands on `/app`; the old `(app)` dashboard routes (incl. `/eventday`) redirect there. The Event-dag cockpit is the desktop variant of the Deur tab.
- `src/components/po/` — kit (`kit.tsx`, `icon.tsx`, `shell.tsx`), shell + nav (`app.tsx`, `context.tsx`, `routes.ts`, `nav-map.ts`), screens under `src/components/po/screens/`. Data via `src/features/po/` (React Query reads over the browser client + `src/features/*` server actions for writes). Desktop density per-screen via the `WIDE_DESKTOP` map in `nav-map.ts`.
- **The shell root reads no venue-wide query, and that is load-bearing for the door (86eykm76k).** `app.tsx` only derives the target from the URL, owns the door's raw-history sub-state and composes three siblings: `app-chrome.tsx` (nav/badges/toasts/`po` context — reads `usePoGuestRequests`/`usePoCanManageTemplates`), `app-screens.tsx` (the screen switch + lazy screen chunks — reads `usePoEvents`), and `door-branch.tsx` (the Deur tab — reads `usePoDoorCandidates`). Moving any of those reads up into `app.tsx` puts a venue-wide refetch back on the door's ancestor path, which is what forced PR #261's twelve hand-memos; `src/components/po/door-render-isolation.test.tsx` fails when it happens. The four shell files carry a hard `max-lines: 800` eslint **error** (`.eslintrc.json`), not a warning.
- **Every screen has a real, bookmarkable URL (G1)** — `src/components/po/routes.ts` is the canonical `screenPath`/`tabPath`/`doorPath` ↔ `parseAppUrl` scheme; `app.tsx` derives the active screen from `usePathname()`/`useSearchParams()` on every render (no in-memory nav stack, no sessionStorage restore-after-refresh hack). Identity/venue resolution + `PoLiveProvider` live in `src/app/app/layout.tsx`, which stays mounted across screen navigations — `[[...segments]]/page.tsx` itself does zero server data work (no `searchParams` read) so query-string-only navigation (door overlay, event picks) stays fully client-side; this matters for the door's offline invariant (#25), not just performance. The layout's consent/MFA/login gates learn the requested deep link for `next=` from the `x-po-request-path` header the middleware always overwrites (`_rsc` stripped); matcher-skipped paths let a client value through, so the layout only ever uses it via `appGateNextPath` (`src/features/auth/next-path.ts`: open-redirect guard + `/app` surface only). The shell mounts via `src/components/po/app-client.tsx` (`ssr: false`, 86eya4yuf) — **never render `PlusOneApp` under a server-streamed Suspense boundary**: `useSearchParams` suspends during SSR and Next 15.5's rAF-gated boundary reveal never hydrates in a tab that hasn't painted (background tab/webview) → the whole app sits on zero-filled SSR HTML with zero fetches, forever. Guards (both in the CI-required `lint-and-test`): `tests/unit/app-shell-no-ssr-suspense.test.ts` (structure — page mounts via `app-client`, `ssr:false` kept) + `tests/e2e/app-home-events-visible.spec.ts` (behaviour — `/app` must render event cards, incl. a `requestAnimationFrame`-stubbed never-painted tab; wired into `pnpm e2e:smoke`). The same trap applies to ANY route root: a client component that suspends during SSR (`useSearchParams`/`use(promise)`/`cookies` in a client boundary) must mount `ssr:false`, never under a page `<Suspense>`.
...
- **Tablet (T1):** iPad ships in the native v1, so every screen must work at 641–1023px before store submission. Content layout switches at `md:` (768px) — never `lg:` for grids/tables — and the `WIDE_DESKTOP` column applies in both chromes. Density follows the pointer: a size below 44px only behind `lg:[@media(pointer:fine)]:` (iPad landscape is desktop width + touch); guarded by `tests/unit/touch-density.test.ts`.
...
**`pnpm dev` runs Turbopack** (86ey9e9zd; escape hatch `DEV_WEBPACK=1 pnpm dev`); `pnpm build` stays webpack. Turbopack has no persistent dev cache, so the stale-`.next` trap above only applies to webpack artifacts; dev startup auto-prunes `.next/cache` past 500 MB (webpack build cache — regenerates on the next `pnpm build`), and `pnpm clean:next` wipes `.next` manually.
...
- **A migration that creates a table or view in `public` must state its grant matrix — `revoke` first, then `grant`.** Since `20260917100000` the default ACL gives `anon` and `authenticated` nothing, so a new object starts closed and the explicit `grant … to authenticated` is what opens it (forget it and you get a loud 403 in dev, not a silent open table). Never express the matrix as `on all tables in schema` — that is a snapshot of today's objects, which is how six objects sat open for three months. `service_role` keeps its defaults. Guarded by `supabase/tests/database/grant_matrix.test.sql` (catalog-driven: `anon` holds nothing, no app role holds TRUNCATE, `authenticated` holds DELETE only on the config/membership allowlist).
...
- **Environment setup is ONE codepath:** `scripts/session-setup.mjs` (inventory → install → check) is what CI, the laptop, and the Claude Code SessionStart hook (`.claude/hooks/session-start.sh` — remote sessions run `install`, local sessions the read-only `inventory`) all run — never re-inline `pnpm install`/suite steps beside it in a workflow or hook. Its suite list is runtime-validated against `package.json` (a from-memory list once named a suite that doesn't exist on main), and `check` ends by naming what it did NOT run. If setup and a guard conflict, the guard wins: never make the app weaker to make the setup easier.
- **English UI copy** (decided 2026-08-19), English code/comments/commit messages. The message catalogue is `src/lib/i18n/` — `en.ts` is the single source of truth and English is the only locale. Voice: `tone-of-voice.md`; string deck: `copy-deck.md`; catalogue design: `ia-audit-claude-code.md` §8. Conventional commits, small commits per logical step.
- **Never write a bare `JSX.Element`** — that reads the *global* JSX namespace, which @types/react 18.3 deprecates and 19 removes (`TS2503`). Import it: `import type { JSX } from 'react';` (or `React.JSX.Element`, or just let TS infer). `@types/react`/`@types/react-dom` are on **19** since z8uq9m0h2h (matching the `react`/`react-dom` 19 runtime), so tsc now rejects a bare `JSX.Element` outright; `tests/unit/jsx-namespace-imported.test.ts` stays as the guard that names the offending file, runs in the unit suite, and keeps holding if the types are ever pinned back to 18 (where the global is only `@deprecated` and tsc goes quiet). Dependabot is configured to ignore next/react/react-dom/@types/react(-dom) **majors** (`.github/dependabot.yml`) — note the measured limit of that rule, documented in the file itself: it does not cover an `@types/*` bump that rides along with its runtime package. Framework majors are their own migration task, never a bump.
