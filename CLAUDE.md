...
- **Device storage is session-scoped unless it is provably PII-free (86ey9e9mn).** `public/service-worker.js` writes to two caches: `plusone-shell-*` (static assets + `/door/<eventId>`, which SSRs no guest data) survives sign-out so the next doorhost still cold-starts offline (#25); `plusone-session-*` (`/app*` — its RSC payload carries user id, venue, roles, name, memberships — plus the `/door` picker) is wiped by `clearDeviceCaches()` from `signOutDevice`, exactly like `idbClearAll()`. Any other navigation is network-only. Adding a path to the persistent cache means asserting its HTML is PII-free on a shared tablet. `public/sw.js` is a self-destructing stub for the retired next-pwa worker — never regenerate a Workbox SW there (it cached cross-origin Supabase REST bodies). Guards: `tests/unit/service-worker-cache-scope.test.ts` + `tests/unit/no-stale-pwa-artifacts.test.ts`.
