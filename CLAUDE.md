...
- Stripe state flows in via **webhooks only** → `POST /api/webhooks/stripe` (the app's only API route; middleware exempts `/api/webhooks/` — auth is the signature). Signature over the raw body; idempotency via the `stripe_webhook_events` ledger + the service_role-only RPC `apply_stripe_subscription_update` — replay mutates nothing. This is the documented service-role exception (security-reviewed 2026-07-06).
