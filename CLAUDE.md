...
- **Never write a bare `JSX.Element`** — that reads the *global* JSX namespace, which @types/react 18.3 deprecates and 19 removes (`TS2503`). Import it: `import type { JSX } from 'react';` (or `React.JSX.Element`, or just let TS infer). tsc can't catch this while `@types/react` is pinned to 18, so the guard is `tests/unit/jsx-namespace-imported.test.ts`. Dependabot is configured to ignore next/react/react-dom/@types/react(-dom) **majors** (`.github/dependabot.yml`); framework majors are their own migration task, never a bump.
