...
- **MFA (TOTP) is OPTIONAL for every role, including `admin` and `finance` (decided 2026-07-01; being built in ClickUp T1 `86ey4j1dz`).** Rationale (Max): forcing MFA creates too much onboarding friction and risks losing customers. It is **recommended, never required** — there is **no hard gate anywhere**: not for app access, and **not for sensitive actions**. We surface a **well-explained recommendation** at the right moments (onboarding, and when a user does a sensitive action such as inviting a teammate): copy like *"We recommend enabling MFA to protect your account. You can skip this if you prefer."* — always with a skip. Skip choices persist on `user_profiles.mfa_snooze_until` (**"Don't ask again"** = never nag; **"Ask me in 7 days"**). Optional roles can still self-enable/disable MFA.
  - **This SUPERSEDES the earlier AAL2-on-sensitive-actions enforcement.** Migration `20260624160000_mfa_scope_sensitive_actions` enforced AAL2 in RLS (`auth.jwt()->>'aal'`) for inviting/revoking a teammate, adding/removing/role-changing a member, and remote-logging-out another user's session. **T1 removes that RLS enforcement** (new migration) so those actions become **role-only** — like quota grants, event-organizer assignment, and audit-log viewing already are. Users who *have* enrolled may still get an optional step-up (`useMfaGate`), but a missing factor no longer blocks anything. **Security trade-off is accepted deliberately** (passwordless OTP already gates account access; MFA is a recommended extra layer, not a wall). ⚠️ Until T1 ships, the *current* code still hard-gates enrollment (`requireAppAccess` → `/mfa/enroll`) and still enforces AAL2 on those actions — so verify against live code before asserting behavior.
