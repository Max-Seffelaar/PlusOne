...
- Dashboard setup + local test-mode script (incl. the iDEAL→SEPA-mandate verification): `docs/stripe-setup.md`. Go-live checklist: ClickUp `86ey6bga8`. Remaining build: PR 2 (checkout/portal buttons + trial nudge), PR 3 (soft-block gating; also add the customer-mismatch guard on the venue-id match path in `apply_stripe_subscription_update` before any Payment Link ever exists — security-review note).
