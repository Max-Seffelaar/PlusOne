...
- **English UI copy**, English code/comments/commit messages. Conventional commits, small commits per logical step.
- **All UI strings live in the message catalogue** (`src/lib/i18n/en.ts`, grouped by surface) — **no inline literals in components.** English is the only locale today; keeping the seam means a second language is a sibling dictionary plus a switch in `src/lib/i18n/index.ts`, not a refactor (ties into #37 Capacitor). Voice rules: `tone-of-voice.md`. String deck: `copy-deck.md`. Note both of those explain themselves in Dutch while their example copy is English — that split is deliberate, not drift.
