...
| 20 | **Authenticatie:** Supabase Auth, passwordless-first (e-mail OTP), invite-only accounts, **MFA optioneel voor álle rollen — aanbevolen, nooit verplicht (verfijnd 2026-07-01, geshipt 2026-07-02 in migratie `20260702120000`; verplichte enrollment voor Admin/Finance en AAL2-op-gevoelige-acties vervallen — forceren kost klanten)**. **Presentatie-verfijning (besloten 2026-07-09, UX/IA 9/7):** de aanbeveling is ask-first — `MfaEnrollCard` toont eerst alleen de uitleg + drie acties ("Set up now" / "Ask me in 7 days" / "Don't ask again"), de QR/verificatie pas na de klik op "Set up now"; op het `/app`-pad gaat de consent-gate (terms/privacy) altijd vóór de MFA-aanbeveling (`/mfa/enroll` zelf heeft geen eigen consent-check — bekende beperking, zie §5); en de aanbeveling verschijnt nooit tot 24 uur na `user_profiles.terms_accepted_at` (niet `user.created_at` — die staat al bij het versturen van de invite) zodat een verse invitee eerst de app ziet. Passkeys zodra Supabase-support stable is. Zie §5. |
...
| 32 | **Billing: Stripe Billing-abonnementen, optioneel voor MVP.** Betaalmethoden: SEPA-incasso + iDEAL (geen creditcard als default). Stripe achter een eigen abstractielaag: de database kent alleen `subscriptions` met een status; Stripe is implementatiedetail, zodat een latere switch (bijv. Mollie bij groot volume) beperkt blijft tot de adapter. Customer portal, dunning en facturen via Stripe. **Verfijning (2026-07-06, fase 13 PR 1 gemerged — PR #109, migratie `20260706120000`):** (a) **prijzen zijn config-gedreven** — bedragen leven uitsluitend in het Stripe-dashboard, de app kent alleen price-IDs uit env (`STRIPE_PRICE_<PLAN>_MONTHLY`); alleen een plan mét price-ID krijgt een checkout-pad (Indie/gratis skipt Stripe volledig, Pro = op aanvraag/handmatig). (b) **Trial = 14 dagen, zachte blokkade**: na afloop zonder betaalmethode banner + admin-features (nieuwe events, invites, import) geblokkeerd tot checkout; deur-app van geplande events en alle data blijven werken. Resterende trial gaat bij checkout mee naar Stripe (`trial_end`). (c) **`past_due`-grace = 14 dagen via Stripe-dunning** (Smart Retries ~2 weken, final action cancel → webhook → `canceled`); geen app-side timers. (d) **`comped` is manual-only** en wordt nooit door webhook-state overschreven (guard in `apply_stripe_subscription_update`); wijzigingen gelogd via audit-trigger op `subscriptions` (actor null = wij). (e) **Webhook-idempotency** via `stripe_webhook_events`-ledger (service_role-only): replay muteert niets. (f) **Store-tax (Apple IAP)**: accountaanmaak is browser-first; de native (Capacitor) schil toont billing read-only zonder checkout/portal/link — seam = `isNativeShell()`. Runbook: `docs/stripe-setup.md`. |
...
| 34 | **Deurbetalingen (fase 2/3, geen MVP):** route A eerst — QR-betaling via Stripe Checkout (iDEAL/Apple Pay op de telefoon van de gast), werkt in de PWA zonder hardware; route B optioneel — Stripe Terminal smart reader via server-driven API; route C later — Tap to Pay op de telefoon van de doorhost, vereist een native schil (Capacitor) + Stripe Terminal SDK + Apple-goedkeuring. Voorbereiding nu beperkt tot: optionele `door_price` per tier, `entry_payments` als latere entiteit, en check-in-flow die een "betaling vereist"-status kan afdwingen. Geen architectuurwijziging nodig. **Verfijning (2026-07-06, feedback 1/7 T3, PR #116):** de tier-editor kreeg een **Free/Paid-toggle** in plaats van één "door price"-veld; een paid tier draagt nu ook een **display-only BTW-%** (`guest_tiers.vat_percent` / `event_template_tiers.vat_percent`, default 9 — NL verlaagd tarief, géén betaalverwerking). Beide template-copyfuncties (`create_event_from_template`/`create_template_from_event`) dragen de kolom mee. Zelfde toggle/kleurenpicker/BTW-veld in de compacte "tier ter plekke aanmaken"-flow (quick-add/bulk/contacts) voor consistentie. |
...
| 43 | **Influencer-aanvraaglinks + attributie-funnel (Requests-epic F1, ClickUp 86ey21vjt, 2026-07-06).** **(a) Influencers:** venue-scoped entiteit (`influencers`: naam, optionele `user_id`-koppeling, optionele handle; soft delete via `archived_at`). Géén event-geankerde gast-PII → valt búiten de retention-job (#29); AVG-verwijdering op verzoek = archiveren + handmatige scrub, zelfde lijn als teamleden. **(b) N aanvraaglinks per event** (`request_links`): elke link hoort bij een influencer óf is label-only ("Instagram bio"), kan een tier vastpinnen, een **max op goedgekeurde headcount** Σ(1+N) dragen (SQLSTATE **45006**, trigger-enforced; removal geeft vrij tenzij al binnen — zelfde geamendeerde #22-regel), **auto-approve** aan hebben (vereist vaste tier), en is pauzeerbaar/verloopbaar. De legacy `events.landing_slug` leeft door als de **default link** (`is_default`; `landing_active` blijft de event-master-switch); één route-namespace: `/e/[slug]` resolvet via `request_links.slug`, bestaande URL's ongewijzigd. Link-slugs zijn immutabel (gedeelde URL's/QR's sterven nooit door een edit). **(c) Auto-approve = systeem-beslissing, eerlijk gelogd (#4/#15):** `decided_via='auto'`, `decided_by` NULL, `guests.added_by` NULL (CHECK: alleen voor landing-gasten mét link-attributie), audit-actor NULL (zoals de anonimiserings-job) — nooit toegeschreven aan de link-maker. Guards (list lock #23, link vol, tier vol, capaciteit, eerder goedgekeurde vingerafdruk) vallen stil terug op een gewone pending-aanvraag. **(d) Attributie overleeft tot de check-in:** `guest_requests.request_link_id` → gekopieerd naar `guests.request_link_id` (immutabel via trigger — geen her-crediteren); check-in-attributie = join via de gast. **(e) Funnel stap 1:** cookie-loze dag-aggregaten per link (`request_link_pageviews_daily`, server-side geteld bij de render, geen IP/UA/PII — statistiek, geen persoonsgegeven). **(f) Gast-statuspagina `/r/[token]`** (amendeert #40(d) en #30: de aanvrager krijgt wél een bevestigingskanaal): bearer-token app-side gegenereerd, DB bewaart alleen de sha256; minimale payload (event, status, +N — géén contact/motivatie/**afwijsreden** — die blijft intern, besluit Max 6-7-2026); retention-job nult de hash (auto-revocatie). **(g) Capped-link-transparantie (test-feedback Max, 6-7-2026):** voor een link mét `max_headcount` toont de publieke pagina bewust het resterende aantal plekken (`get_landing_event.spots_left`): de stepper is gecapt op wat de link nog kan goedkeuren en een volle link toont "The list is full." — een bewuste versmalling van de #28-no-enumeration voor gecapte links (aanvragen verzamelen die nooit goedgekeurd kunnen worden is slechter dan dit prijsgeven). Links zónder max blijven volledig ondoorzichtig (spots_left = NULL). De submit-RPC houdt z'n eigen guards (een race langs de pagina valt alsnog terug op pending). Verder uit dezelfde ronde: geen "Add someone else" na een instant-goedkeuring, en het audit-log resolvet de aanvrager-naam (audit_feed.request_name) + benoemt auto-goedkeuringen expliciet als systeem-actie via de link. **Fase 2 (gebouwd 7-7-2026, design-first via Claude Design S15/S16 — `docs/design/`):** intern **Promotion**-scherm (po `promo`: overview-funnel per event, "Who actually delivers"-leaderboard 30/90/all met paginering, losse "No influencer"-label-links, create-link-flow) op SECURITY INVOKER-RPC's `event_link_funnel` / `venue_influencer_leaderboard` / `venue_label_link_funnel` (RLS = grens, staff ziet nul rijen), en de publieke **influencer-statspagina `/i/[token]`** (`get_influencer_stats`: bearer-token sha256, throttled `if:`, aggregaten + per event de eígen link-slug voor copy/QR); token mint/rotate/revoke door de admin op het Influencers-scherm, éénmalig getoond. Fase 3 = publieke pagina-branding + transactionele mail via Resend achter een MailProvider-seam (86ey6b3hv — transactionele statusbevestiging ≠ outbound invitation, verduidelijkt CLAUDE.md-regel 10). |
| 44 | **Canonieke telregels — één gedeelde headcount-selector overal (UX/IA 8/7 M4, K-10, ClickUp `86ey7dzdc`).** Vóór deze beslissing had elk oppervlak zijn eigen telling; die dreven uit elkaar (K-10: cockpit zei "4/38 · 34 onderweg", mobiele deur + EventView zeiden "37 · 33" — zelfde event, zelfde moment). Vanaf nu geldt overal dezelfde definitie: **On the list** = Σ(1 + plus_ones) over gasten met status ≠ `removed` en ≠ `refused` (koppen, +1's inbegrepen) · **Inside** = ingecheckte koppen, bij een partial check-in tellen alleen de daadwerkelijk gearriveerde metgezellen · **On the way** = On the list − Inside · **Refused/Bounced** telt in geen van bovenstaande mee, uitsluitend het eigen aparte cijfer · **Attendance/turnout** = Inside ÷ On the list (0% bij lege lijst) · waar zowel gasten (rijen) als koppen (mensen) getoond worden, altijd expliciet gelabeld ("24 guests · 33 people"). Geldt voor deur (mobiel), cockpit, EventView, Home-kaarten en Analytics; de publieke `/e/[slug]`-pagina toont een ander cijfer (`spots_left`, link-capaciteit) en valt buiten deze regel. **Implementatie:** één canonieke client-selector `src/features/po/headcount.ts` (`computeHeadcounts`) — de cockpit (`eventday/cockpit.ts`) en de deur (`door/model.ts`) delegeren er nu allebei aan i.p.v. elk hun eigen reducer te onderhouden (dat was precies waardoor ze uit elkaar konden groeien). **Root cause K-10, twee bugs gevonden en gefixt (migratie `20260710120000`):** (a) de aggregate-RPC's `event_stats_summary`/`event_tier_stats`/`venue_stats_summary`/`venue_event_rollup` telden refused-gasten wél mee in "registered" (een bewuste maar inmiddels achterhaalde keuze) — nu uitgesloten, `refused` blijft een eigen, direct op `guests.status` gebaseerd cijfer; `event_user_additions`/`venue_user_additions` blijven bewust ongewijzigd (een ander, "gross incl. removed"-attributiecijfer, geen live telling). (b) `venue_event_headcounts.present` (Home/EventView) telde bij een partial check-in de volledige geregistreerde groep i.p.v. de daadwerkelijk gearriveerde koppen — gefixt. **Bijvangst:** de realtime-subscriptions op `check_ins` (zowel cockpit `usePoEventRealtime` als deur `useDoorSync`) filterden op `event_id`, een kolom die `check_ins` niet heeft — check-in-realtime werkte dus helemaal niet (alleen de 60s safety-sync ving het op); de filter is verwijderd (deur filtert client-side op gast-lidmaatschap, zoals al bedoeld; cockpit invalideert nu ongefilterd — goedkoop en idempotent). |
...
entry_payments [later]— deurbetalingen per gast: bedrag, methode (qr/reader/
                        ...
                        guest_tiers heeft al door_price_cents + vat_percent
                        (display-only, géén betaalverwerking, T3 2026-07-06)
...
| Invite-status & resend (T8, 2026-07-07) | **Team-scherm toont per invite pending/expired/accepted + "Resend"** | Resend = nieuwe invite-mail + verse 7-dagen-expiry. De expiry-bump is de enige client-UPDATE op `invites` ooit: column-grant beperkt tot `expires_at`, RLS `invites_update_resend` (migratie `20260707113000`) — manager-rol, alleen pending, escalation guard, max 30 dagen vooruit; audit-trigger logt elke resend. External crew (direct gegrant via `event_organizers`, geen invite-rij) staat als eigen sectie op het Team-scherm; "nog niet ingelogd" is afgeleid van `user_profiles.terms_accepted_at`, resend = nieuwe login-mail (admin-only). Crew-invite verstuurt sinds T8 ook daadwerkelijk een mail (`inviteUserByEmail` i.p.v. `createUser`, dat nooit mailde). |
| MFA (alle rollen, incl. Admin & Finance) | **Optioneel — aanbevolen, nooit verplicht (verfijnd 2026-07-01, geshipt 2026-07-02)** | Via Supabase native MFA (TOTP: QR + 6-cijferige code), voor iedereen zelf in/uit te schakelen; het profiel toont "Recommended for your role" voor admin/finance (S4.3). **Geen harde gate meer**: niet voor app-toegang en niet voor gevoelige acties — de eerdere verplichte enrollment (Admin/Finance) én de AAL2-RLS-afdwinging op teamlid uitnodigen / invite intrekken / lid toevoegen-verwijderen-rol wijzigen / remote uitloggen (verfijning 2026-06-24) **zijn vervallen; die acties zijn role-only** (migratie `20260702120000_mfa_fully_optional` trekt de checks uit `20260624160000` in). Rationale: MFA forceren geeft zo veel onboarding-frictie dat het klanten kost; passwordless OTP gate't de account-toegang al. In plaats daarvan een **goed uitgelegde, skipbare aanbeveling** bij app-entry: *"We recommend enabling MFA to protect your account. You can skip this if you prefer."* — met "Don't ask again" of "Ask me in 7 days" (`user_profiles.mfa_snooze_until`). Bewust geaccepteerde security-afweging. **Ask-first presentatie (verfijnd 2026-07-09, UX/IA 9/7):** de aanbeveling is een twee-staps-scherm — stap 1 toont alleen de uitleg + de drie acties, de QR/manual-secret/verificatie pas ná de klik op "Set up now" (geen auto-enroll meer on mount, dus geen half-aangemaakte factor voor wie het scherm alleen zag); op het `/app`-pad gaat de terms/privacy-consent-gate altijd vóór de MFA-aanbeveling (live pad: `src/app/app/layout.tsx`; `requireAppAccess` in `guards.ts` documenteert dezelfde volgorde maar heeft vandaag geen aanroeper) — `/mfa/enroll` zelf heeft géén eigen consent-check, dus een diepe link kan een niet-geconsente gebruiker daar alsnog laten enrollen/snoozen; bekende beperking, apart weggezet als vervolgtaak, niet in deze PR opgelost; en de aanbeveling verschijnt nooit tot 24 uur na `user_profiles.terms_accepted_at` (`recommendMfaIfDue` toetst dat veld, niet `user.created_at` — dat staat al bij het versturen van de invite, dagen vóór de eerste keer inloggen) zodat een verse invitee eerst de app ervaart. Zelf inschakelen via Profile (S4.3) blijft altijd meteen beschikbaar. Backlog-tegenwicht: per-venue policy "require MFA" (taak `86ey4uv97`) zodat een venue het alsnog kan afdwingen — met copy die duidelijk maakt dat de eis van de venue-admin komt. |
