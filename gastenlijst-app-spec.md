...
| 20 | **Authenticatie:** Supabase Auth, passwordless-first (e-mail OTP), invite-only accounts, **MFA optioneel voor álle rollen — aanbevolen, nooit verplicht (verfijnd 2026-07-01; verplichte enrollment voor Admin/Finance en AAL2-op-gevoelige-acties vervallen — forceren kost klanten)**, passkeys zodra Supabase-support stable is. Zie §5. |
...
| MFA (alle rollen, incl. Admin & Finance) | **Optioneel — aanbevolen, nooit verplicht (verfijnd 2026-07-01)** | Via Supabase native MFA (TOTP: QR + 6-cijferige code), voor iedereen zelf in/uit te schakelen. **Geen harde gate meer**: niet voor app-toegang en niet voor gevoelige acties — de eerdere verplichte enrollment (Admin/Finance) én de AAL2-RLS-afdwinging op teamlid uitnodigen / invite intrekken / lid toevoegen-verwijderen-rol wijzigen / remote uitloggen (verfijning 2026-06-24) **vervallen; die acties worden role-only** (migratie in ClickUp T1 `86ey4j1dz` trekt de checks uit `20260624160000` in). Rationale: MFA forceren geeft zo veel onboarding-frictie dat het klanten kost; passwordless OTP gate't de account-toegang al. In plaats daarvan een **goed uitgelegde aanbeveling** op de juiste momenten (onboarding + bij bv. een teamlid uitnodigen): *"We recommend enabling MFA to protect your account. You can skip this if you prefer."* — altijd skipbaar met "Don't ask again" of "Ask me in 7 days" (`user_profiles.mfa_snooze_until`). Bewust geaccepteerde security-afweging. Backlog-tegenwicht: per-venue policy "require MFA" (taak `86ey4uv97`) zodat een venue het alsnog kan afdwingen — met copy die duidelijk maakt dat de eis van de venue-admin komt. |
