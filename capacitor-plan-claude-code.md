...
| Legal | 🔶 L1 | `src/lib/legal.ts` linkt naar `plusone.app/terms` en `/privacy`; die pagina's bestaan nog niet (drafts in `docs/legal/`). Beide stores eisen een live privacy-policy-URL bij submissie, en de consent-gate in de app linkt er al naartoe → [86ey1vbrj](https://app.clickup.com/t/86ey1vbrj) is een harde S5-dependency. |
...
| L1 | Legal-pagina's live | SHIP | 1 | — | `/privacy` + `/terms` publiceren vanuit `docs/legal/` (na Max' juridische check), `NEXT_PUBLIC_*_URL` in Vercel zetten, consent-gate-links verifiëren. → [86ey1vbrj](https://app.clickup.com/t/86ey1vbrj) (bestaand, prod-ready-programma). |
...
| 3 | N4, N5, S2, S4, S1a | N3 gemerged (S1a ook M2) |
...
| CSP-check bij wrap | Aangescherpt: strikte CSP bevestigd; Android-injectie valt onder `script-src`. | N3 |
