...
**STAP 4.1** Uitvoerbare test-suites (geen niet-bestaande features) sequentieel draaien + rapporteren.
  → ✅ done 2026-06-23 → `docs/test-report.md`. Sequentieel groen: **Vitest 39 files/434 tests** + **pgTAP 22 files/529 tests** op een verse `supabase db reset` (bewijst: geen migratie-timestamp-collisies), `pnpm lint` + `type-check` schoon. Gaten gedicht voor bestaande logica: offline-outbox `refusal`/`ack_note` replay (nu 8/8 kinds) + statushelpers (`isPending`/`isRetryable`/`hasUnsynced`), en een nieuwe **secret-grep** guard (`service_role`-key confinement — faalt de suite als de key buiten de éne `server-only`-module of in een `'use client'`-component opduikt; draait in de bestaande `pnpm test`-CI-stap). Quota-math/+N, lock en de quick-add parser bleken al dekkend. **Uitgesteld** (feature bestaat nog niet): Stripe-webhook, ticketing-adapter/Vault, deurtaken-realtime/push. Open gaten: geen line-coverage-meting; server-`actions.ts` alleen indirect gedekt (RLS = de grens); e2e-kernflow = STAP 4.3.
