...
// The source and tier pills are 37–40px; an invisible 4px ring above and below
// gives ≥44px on touch without changing their look (T1, iPad = touch). Wrapped
// rows sit 8px apart, so two rows' rings meet without overlapping.
const pillHit = "relative before:absolute before:-inset-y-[4px] before:inset-x-0 before:content-['']";
    ...
    <div className="mt-5 rounded-[18px] border border-line bg-elev p-4">
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => setTierId('')}
          className={cn('inline-flex items-center gap-[7px] rounded-full border px-[13px] py-[8px] font-display text-[12.5px] font-bold', press, pillHit, tierId === '' ? 'border-transparent bg-acc text-on-acc' : 'border-line text-dim')}
        >
        ...
        {tiers.map((tier) => {
          ...
          return (
            <button
              ...
              className={cn('inline-flex items-center gap-[7px] rounded-full border px-[13px] py-[8px] font-display text-[12.5px] font-bold', press, pillHit, on ? 'border-transparent bg-acc text-on-acc' : 'border-line text-dim')}
            >
