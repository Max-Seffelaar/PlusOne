...
export function TabBar({
  ...
  // The bar spans the full width, but its items sit in a centered 640px row:
  // on a tablet (641–1023px, T1) five `flex-1` tabs would otherwise land
  // ~200px apart — the iPad tab-bar convention is a centered cluster.
  return (
    <div
    ...
    <div className="po-anim-fade absolute inset-0 z-40 flex items-end justify-center bg-[rgba(6,6,8,0.66)] p-4 backdrop-blur-[3px]" onClick={onClose}>
      <div
        className={cn(
          ...
          // max-w: on a tablet or a wide desktop column a full-width bottom
          // sheet would stretch 800–1000px; cap it to a dialog width (T1). A
          // phone never reaches the cap.
          'po-anim-sheet flex max-h-full w-full max-w-[560px] flex-col overflow-y-auto rounded-sheet border border-line bg-elev p-[22px] shadow-[0_-20px_60px_rgba(0,0,0,0.5)]',
