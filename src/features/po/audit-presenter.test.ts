...
describe('auditActionMeta', () => {
  it('maps known actions to an icon + Dutch label', () => {
  ...
  it('falls back for unknown actions', () => {
    expect(auditActionMeta('something_new')).toEqual({ icon: 'history', label: 'Actie' });
  });
});

describe('guestStatusLabel', () => {
  it('translates known statuses', () => {
    expect(guestStatusLabel('checked_in')).toBe('Binnen');
    expect(guestStatusLabel('removed')).toBe('Verwijderd');
  });
...
describe('AUDIT_ACTION_FILTERS', () => {
  it('starts with the "all" reset chip', () => {
    expect(AUDIT_ACTION_FILTERS[0]).toEqual({ key: 'all', label: 'Alles' });
  });
});
