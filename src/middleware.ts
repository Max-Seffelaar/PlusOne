...
export const config = {
  // Run on everything except Next internals and static assets. `monitoring(?:/|$)`
  // is the Sentry tunnel route (D2): it MUST stay excluded — otherwise the
  // auth-gate 307's every ingest envelope to /login and no event ever arrives
  // (the plan's #1 silent failure mode). The `(?:/|$)` anchor matches exactly
  // `/monitoring` and `/monitoring/<subpath>` but NOT `/monitoring-*` — a bare
