...
-- to be RAISED over time via the door's "nog inchecken" action. The update is
-- scoped to the checker's own row by the existing `check_ins_update_own_device`
-- policy (checked_by = auth.uid()).
--
...
--     a stale absolute target can never lower it (idempotent increments).
--
-- The first check-in's identity + time stay put: the increment path writes only
-- `plus_ones_arrived`, and the AFTER audit trigger (decision #4) records the diff.
