...
-- === AUDIT TRIGGER FUNCTION (created early so it captures seed data) ===
...
-- Create triggers on all mutable entities
create trigger guests_audit after insert or update or delete on guests for each row execute function audit_trigger();
create trigger guest_tiers_audit after insert or update or delete on guest_tiers for each row execute function audit_trigger();
create trigger quotas_audit after insert or update or delete on quotas for each row execute function audit_trigger();
create trigger event_quotas_audit after insert or update or delete on event_quotas for each row execute function audit_trigger();
create trigger quota_requests_audit after insert or update or delete on quota_requests for each row execute function audit_trigger();
create trigger check_ins_audit after insert or update or delete on check_ins for each row execute function audit_trigger();
create trigger refusals_audit after insert or update or delete on refusals for each row execute function audit_trigger();
create trigger venue_memberships_audit after insert or update or delete on venue_memberships for each row execute function audit_trigger();
create trigger events_audit after insert or update or delete on events for each row execute function audit_trigger();
